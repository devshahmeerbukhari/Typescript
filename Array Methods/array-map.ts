...
function squareOfEachElement(array: number[]){
...
function stringToNumber(array: string[]){
...
let strToNumArr: any[] = stringToNumber(array2)
...
function extractName(array: any[]){
    return array.map((element) => element.name)
}
const extractNameArr: string[] = extractName(array3)
...
const products = [
  ...
  const discountedProducts = products.map(product => ({
    ...product,                 // Spreads the properties of the product object into the new object
    price: product.price * 0.9  // Overrides the price property with the discounted price
    /*
    The '...product' syntax is called the "spread operator" in JavaScript (and TypeScript). 
    It allows you to copy all properties from the product object into a new object. 
    This is useful for creating a new object that includes all the properties of the 
    existing product object, but allows you to override or add specific properties.
    */
  }));
